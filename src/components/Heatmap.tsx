...
export default function Heatmap({ filters: propFilters }: HeatmapProps) {
  ...
  const fetchHeatmapData = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8000/api/resistance-heatmap/${urlSuffix}`, { headers });
            ...
            <table className="min-w-full border-collapse">
              <thead>
                <tr>
                  ...
                  {uniqueAntibiotics.map(antibiotic => (
                    <th key={antibiotic} className="border p-2 bg-muted text-xs">
                      {antibiotic}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {uniqueBacteria.map(bacteria => (
                  <tr key={bacteria}>
                    <td className="border p-2 font-medium bg-muted/50">{bacteria}</td>
                    {uniqueAntibiotics.map(antibiotic => {
                      const data = heatmapData.find(d => d.bacteria === bacteria && d.antibiotic === antibiotic);
