...
export default function Reports() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      let url = `${process.env.NEXT_PUBLIC_API_URL}/api/reports/monthly/`;
      ...
      if (
        ...
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/reports/specific/`;
        ...
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/reports/bacteria/`;
        params.bacteria_id = String(selectedBacteria);
      } else if (reportScope === "all_antibiotics" && selectedAntibiotic) {
        url = `${process.env.NEXT_PUBLIC_API_URL}/api/reports/antibiotic/`;
        params.antibiotic_id = String(selectedAntibiotic);
      }
      ...
      if (
        ...
        const bacteriaName =
          bacteria.find((b) => b.id === selectedBacteria)?.name || "bacteria";
        const antibioticName =
          antibiotics.find((a) => a.id === selectedAntibiotic)?.name ||
          "antibiotic";
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            >
              ...
              <option value="all_antibiotics">
                All Antibiotics (by Bacteria)
              </option>
            </select>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-2">
              {reportScope === "all_antibiotics"
                ? "Select Bacteria (Optional - leave empty for all)"
                : "Select Bacteria"}
            </label>
            <select
          ...
          <div>
            <label className="block text-sm font-medium mb-2">
              {reportScope === "all_bacteria"
                ? "Select Antibiotic (Optional - leave empty for all)"
                : "Select Antibiotic"}
            </label>
            <select
          ...
          <div className="p-3 bg-muted rounded-md text-sm">
            ...
            <ul className="mt-1 text-muted-foreground">
              {reportScope === "specific" &&
                ...
                selectedAntibiotic && (
                  <li>
                    • Specific report for selected bacteria-antibiotic
                    combination
                  </li>
                )}
              {reportScope === "specific" &&
